...
class LearningPlan(ObjectBaseClass):
    ...
    uni_title = models.CharField(_("Название"), max_length=1024, blank=True, null=True)
...
class ProgramModules(ObjectBaseClass):
    ...
    def get_all_discipline_custom(self):
        # """Delete not unique"""
        # q = [[s for s in Semester.objects.filter(discipline=discipline, program=self.program).order_by("term")] for discipline in Discipline.objects.filter(module=self.module, archived=False, status="p").order_by("period")]
        # for_delete = []
        # for sem in q:
        #     title = None
        #     trsemester = 0
...
class Changed(models.Model):
    ...
    view = models.CharField(max_length=1024, blank=True, null=True)
