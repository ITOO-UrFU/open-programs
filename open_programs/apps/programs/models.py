...
class ProgramModules(ObjectBaseClass):
    ...
    def get_target_positions(self):
        targets_positions = []
        try:
            tr_targets = TrainingTarget.objects.filter(program=self.program).order_by('number').values("id", float=True)
            for tt in tr_targets:
                tms = TargetModules.objects.filter(program_module=self, target__id=tt, status="p",
                                                   archived=False)
                if not tms:
                    status = 0
